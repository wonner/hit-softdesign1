   ...
   public void paintComponent(Graphics g)
   {
      ...
      for(int i=0;i<this.graph.getN();i++)
      {
    	  ...
    	  while(temp!=null)
    	  {
    		  ...
    		  {
    			  EdgeNode tmp=this.graph.getVertexNode(temp.getIndex()).getFirstedge();
        		  while(true)
        		  {
        			  if(tmp.getIndex()==i)
        			  {
    		  ...
    		  g2.setPaint(Color.BLACK);
    		  g2.drawString(Double.toString(temp.getWeight()),(int)(x[i]+x[temp.getIndex()])/2, (int)(y[i]+y[temp.getIndex()])/2);
    		  temp=temp.getNext();
    	  }
      }
